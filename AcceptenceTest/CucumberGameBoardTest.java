...
public class CucumberGameBoardTest {
    ...
    public void theTileIsPlacedinaNonemptyspot() throws GameRulesException{
        tile = deck.getTile();
        tileLocations[0] = new Point(5, 0);
        tileLocations[1] = new Point(4, 0);
        tileLocations[2] = new Point(4, 1);
        try {
            rules.TryToAddTile(tile, tileLocations);
        }catch (GameRulesException e) {
            Assert.assertEquals("The Tile Is Not in an empty spot", e.getMessage());
        };
    }
    @Then("^the tile is not added to the map$")
    public void theTileIsNotAddedToTheMap2() throws GameRulesException{
        try {
            rules.TryToAddTile(tile, tileLocations);
        }catch (GameRulesException e) {
            Assert.assertEquals("The Tile Is Not in an empty spot", e.getMessage());
